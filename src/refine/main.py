...
async def atlassian_is_relevant(client: BedrockClient, uri: str, query: str) -> bool:
...
async def process_directory(client: BedrockClient, directory: str, query: str) -> list[str]:
    relevant_uris = []
...
async def file_is_relevant(client: BedrockClient, file_name: str, query: str) -> bool:
...
async def text_is_relevant(client: BedrockClient, text: str, query: str) -> bool:
    chunks = await client.chunk(text)
    tasks = [client.relevant_text(chunk, query) for chunk in chunks]
    results = await asyncio.gather(*tasks)
    return any(results)

async def url_is_relevant(client: BedrockClient, uri: str, query: str) -> bool:
...
async def text_file_is_relevant(client: BedrockClient, file_name: str, query: str) -> bool:
...
async def image_is_relevant(client: BedrockClient, file_name: str, query: str) -> bool:
    with Image.open(file_name) as image:
        return await client.relevant_image(image, query)


async def uri_is_relevant(client: BedrockClient, uri: str, query: str) -> bool:
